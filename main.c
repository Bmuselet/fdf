...
/*   Updated: 2017/12/04 19:37:50 by bmuselet         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void ft_ligne(void *win, void *mlx, int x1, int y1, int x2,int y2) 
{
	...
	if (x1 <= x2 && (x2 - x1) >= abs(y2-y1))
	{
		...
		while (x <= x2)
		{            
			mlx_pixel_put(mlx, win, x, y1 + ((y2 - y1) * (x - x1)) / (x2 - x1), ORANGE);
			x++;
		}
	}
	if (x2 <= x1 && (x1 - x2) >= abs(y1-y2))
	{
		...
		while (x <= x1)
		{
			mlx_pixel_put(mlx, win, x, y2 + ((y1 - y2) * (x - x2)) / (x1 - x2), WHITE);
			x++;
		}
	}
	else if (y1 <= y2 && (y2 - y1) >= abs(x2-x1))
	{
		...
		while (y <= y2)
		{            
			mlx_pixel_put(mlx, win, y, x1 + ((x2 - x1) * (y - y1)) / (y2 - y1), GREEN);
			y++;
		}
	}
	else if (y2 <= y1 && (y1 - y2) >= abs(x1-x2))
	{
		...
		while (y <= y1)
		{
			mlx_pixel_put(mlx, win, y, x2 + ((x1 - x2) * (y - y2)) / (y1 - y2), BLUE);
			y++;
		}
	}
}
...
void ft_pos(int i, int j, int z, int* outx, int* outy)
{
	...
	*outy = starty + (i * pas / 4) + (j * pas / 4) - (z * 30);
}
...
void	ft_segment(void *win, void *mlx, int xmin,int ymin,int xmax,int ymax, int z)
{
	...
	mlx_pixel_put(mlx, win, imin, jmin, WHITE);
